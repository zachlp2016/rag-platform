"""Non-production reference evaluator for the semantic routing contract."""
...
def resolve(
    ...
    role_index, provider_index = _identity_indexes(manifest)

    normalized_role = normalize_identity(role_key)
    normalized_provider = normalize_identity(provider_name)
    role_route = role_index.get(normalized_role) if normalized_role else None
    provider_route = (
        provider_index.get(normalized_provider) if normalized_provider else None
    )
